...
steelCanvas = gr.material_extended({0.5, 0.5, 0.6}, {1.0, 1.0, 1.0}, 1000)
groundCanvas = gr.material_extended({0.65, 0.65, 0.65}, {0.8, 0.5, 0.3}, 80)
woodCanvas = gr.material_extended({0.5, 0.5, 0.6}, {0.42, 0.32, 0.0}, 30)
pillarCanvas = gr.material_extended({0.5, 0.5, 0.6}, {0.75, 0.75, 0.7}, 200)
ringCanvas = gr.material_extended({0.8,0.8,0.8}, {0.3,0.3,0.3}, 100)
...
primary_light = gr.light({-8000, 8000, 0}, {0.45, 0.3, 0.1}, {1, 0, 0})
backlight = gr.light({0, 400, 400}, {0.7,0.7,0.7}, {1, 0, 0})
flame1 = gr.light({-55,-25,-55}, {0.46, 0.23, 0.1}, {1,0,0.00011})
flame2 = gr.light({-55,-25,-145}, {0.46, 0.23, 0.1}, {1,0,0.00011})
flame3 = gr.light({-55,-25,-235}, {0.46, 0.23, 0.1}, {1,0,0.00011})
flame4 = gr.light({62,-25,-55}, {0.46, 0.23, 0.1}, {1,0,0.00011})
flame5 = gr.light({62,-25,-145}, {0.46, 0.23, 0.1}, {1,0,0.00011})
flame6 = gr.light({62,-25,-235}, {0.46, 0.23, 0.1}, {1,0,0.00011})
...
s_helmet:set_bumps(t_smoothsteel, 'rgb')
...
s_helmet_inner:set_material(steelCanvas)
...
s_helmet_eyehole:set_material(steelCanvas)
...
s_helmet_mouthhole:set_material(steelCanvas)
...
thigh:set_bumps(t_smoothsteel, 'rgb')
...
shin:set_bumps(t_smoothsteel, 'rgb')
leg:add_child(shin)
...
shoe_differ:set_bumps(t_gloves, 'rgb')
...
arm_top:set_bumps(t_smoothsteel, 'rgb')
...
arm:set_bumps(t_smoothsteel, 'rgb')
elbow:add_child(arm)
------------
...
blade_edge_carver:set_material(turquoise)
...
diag_left:set_material(turquoise)
...
diag_right:set_material(turquoise)
...
diag_front:set_material(turquoise)
...
diag_back:set_material(turquoise)
...
--gr.render(scene, 'FinalScene.png', 888, 500, {0, 0, 100}, {0, 0, -100}, {0, 1, 0}, 50, {0.2, 0.2, 0.2}, {primary_light, flame1, flame2, flame3, flame4, flame5, flame6, backlight})
gr.render(scene, 'FinalScene.png', 888, 500, {0, 0, 100}, {0, 0, -100}, {0, 1, 0}, 50, {0.2, 0.2, 0.2}, {primary_light, testlight, backlight})
