...
cylinder_x:set_texture(texture_checkerboard)
cylinder_x:set_material(perfectCanvas)

cylinder_y = gr.cylinder('cylinder_y')
cylinder_y:scale(1/50,1/50,1/100)
cylinder_y:scale(3,10,6)
cylinder_y:translate(0,0.13,-0.1)
cylinder_y:translate(0.08,0.2,.5)
cylinder_y:set_texture(texture_checkerboard)
cylinder_y:set_material(perfectCanvas)
...
cylinder_z:set_texture(texture_checkerboard)
cylinder_z:set_material(perfectCanvas)
...
sphere:set_material(polgold)

xy = gr.csg('xy')
xy:set_csg_children(cylinder_x, cylinder_y, 'UNION')

xyz = gr.csg('xyz')
xyz:set_csg_children(xy, cylinder_z, 'UNION')
...
cubeWithoutCore:set_csg_children(sphere, xyz, 'DIFFERENCE')
...
center_rod:set_texture(texture_bricks)
center_rod:set_material(perfectCanvas)


conicsg = gr.csg('conicsg')
conicsg:set_csg_children(top_cone, bottom_hyperboloid, 'INTERSECT')
